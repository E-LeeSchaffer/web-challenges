...
function calculateScrollPercentage() {
  const currentScrollerHeight = window.scrollY;
  const windowHeight = window.innerHeight;
  const webpageHeight = document.body.clientHeight;
  // maximale Scrollhöhe
  const scrollableHeight = webpageHeight - windowHeight;
  const scrollPercentage = (currentScrollerHeight / scrollableHeight) * 100;
